...
### 2.4 `POST /api/admin/logout`

- 功能描述：管理员登出，撤销当前会话使已签发的 token 失效。
- 使用场景：后台点击“退出登录”。
- 鉴权：需要管理员 token（Bearer）。
...
说明：该接口会将 `AdminUser.currentSessionId` 清空，配合服务器端的 `sid` 校验逻辑使当前及旧的含 `sid` 的 JWT 失效。客户端应在收到成功响应后同时清除本地存储的 token（localStorage / cookie 等）。

错误码：`401`（未授权或 token 无效）、`403`（非管理员）、`500`（服务器错误）
